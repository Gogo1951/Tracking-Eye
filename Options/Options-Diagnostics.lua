...
function ns.BuildDiagnosticsOptions()
    return {
        ...
        args = {
            ...
            headerPlayer = SectionHeader(D.PLAYER_TITLE, 24),
            buttonPlayer = {
                type = "execute",
                ...
                hidden = Hidden,
                func = function()
                    ns.diagnostics.playerReport = ns:BuildPlayerContextReport()
                    Refresh()
                end
            },
            outputPlayer = ReportOutput("playerReport", 26),

            -- Display Context
            headerDisplay = SectionHeader(D.DISPLAY_TITLE, 27),
            buttonDisplay = {
                type = "execute",
            ...
            outputDisplay = ReportOutput("displayReport", 29),

            -- Farm Mode Context
            headerFarmContext = SectionHeader(D.FARM_TITLE, 29.5),
            buttonFarmContext = {
                type = "execute",
                name = D.FARM_BUTTON,
                order = 29.6,
                hidden = Hidden,
                func = function()
                    ns.diagnostics.farmReport = ns:BuildFarmContextReport()
                    Refresh()
                end
            },
